...
def load_weights_from_FaceNet(FRmodel):
    # Load weights from csv files (which was exported from Openface torch fd_models)
    weights = WEIGHTS
    weights_dict = load_weights()

    # Set layer weights of the fd_models
    for name in weights:
        if FRmodel.get_layer(name) != None:
            FRmodel.get_layer(name).set_weights(weights_dict[name])
        # elif fd_models.get_layer(name) != None:
        #     fd_models.get_layer(name).set_weights(weights_dict[name])
